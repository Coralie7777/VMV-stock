...
- **Rôles utilisateurs** : `USER_ROLE` vient d'Airtable (VMV Utilisateurs).
  Tous les onglets (Stock, BL, Chantiers, Scan, Alertes, Export) sont
  visibles quel que soit le rôle. `isAtelier()` (nom d'utilisateur ou rôle
  contenant "atelier") sert uniquement à masquer le champ remise dans un
  formulaire — pas à cacher des onglets. Le rôle `Admin` fait apparaître un
  onglet `tab-admin` normalement caché.
...
- **Scan** : douchette QR/code-barres via ZXing
  (`cdnjs.cloudflare.com/ajax/libs/zxing-js/0.20.0/zxing.min.js` — seule
  source CDN confirmée fonctionnelle, ne pas changer sans tester).
